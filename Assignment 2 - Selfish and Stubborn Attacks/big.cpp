...
void outpFracOfBlocksInLongestChain(int n, int attacker){
    cout<<setprecision(3);
    ...
    cout<<"Total generated by attacker = "<<total_blocks_generated[0]<<endl;
    double slow_sum=0;
    double fast_sum=0;
    int num_slow=0;
    int num_fast=0;
    double frac_attacker=0;
    ...
    for(iter=id_node_mapping.begin(); iter!=id_node_mapping.end(); iter++){
        ...
        int length=0;
        while(btn){
            if(node->node_id==attacker && node->forked_chain_unreleased.find(btn->block_id)!=node->forked_chain_unreleased.end()){
                cout<<"Unrelased "<<btn->block_id<<endl;
                btn = btn->parent;
                continue;
            }
        	length++;
...
//  individual contri of nodes to longest chain
        ...
        if(node->node_id==attacker){
            cout<<"MPU_attacker (node "<<attacker<<") : "<<(1.0*contri_nodes[attacker])/total_blocks_generated[attacker]<<endl;
            frac_attacker=(1.0*contri_nodes[attacker])/length;
        }

    }
    // mpu_overall
    cout<<"MPU_overall : "<<(1.0*id_node_mapping[1]->longest_chain_head->level)/total_blocks<<endl;
    
    cout<<"Fraction attacker nodes in longest chain = "<<frac_attacker<<endl;
    // if(num_slow !=0)
...
int main()
{
   ...
   ///////////////////// differentiate between high and low hashing power/////////////////////////
    ...
    //  % of hashing power with attacker
   double alpha=0.4;
   ...
   for(int i=0;i<n;i++){
    ...
    tot_sum-=rand_hashing_power[attacker];
    cout<<"Rest total "<<tot_sum<<endl; 
    rand_hashing_power[attacker]=(alpha*tot_sum)/(1-alpha);
    cout<<"Attacker "<<rand_hashing_power[attacker]<<endl;
//    rand_hashing_power[attacker] = tot_sum*0.6;
   ...
   //decide a random node to be given credit for genesis block having a transaction giving it 50 coins
    ...
    cout<<"T_tx = "<<T_tx<<endl;
    cout<<"T_k = "<<T_k<<endl;
    cout<<"Attacker hashing power : " <<id_node_mapping[attacker]->hash_percent<<" %"<<endl;
    cout<<"Fraction neighbors of attackers : "<<frac_neighbours<<endl;
    if(id_node_mapping[attacker]->is_attacker_selfish)
        cout<<"Selfish mining"<<endl;
    if(id_node_mapping[attacker]->is_attacker_stubborn)
        cout<<"Stubborn mining"<<endl;
